...
struct Day6 {
    ...
    static let groupedAnswers = rawInput
        .splitDoubleNewlines()
        .map {
            AnswerGroup(rawLines: $0)
        }
    
    struct AnswerGroup {
        var positiveResponses = Set<Character>()
        var unanimousResponses = Set<Character>()
        init(rawLines: Substring) {
            var responseCounts = [Character: Int]()
            let participantResponses = rawLines.split(whereSeparator: \.isNewline)
            for line in participantResponses {
                for c in line {
                    positiveResponses.insert(c)
                    responseCounts[c, default: 0] += 1
                }
            }
            unanimousResponses = Set(responseCounts
                .filter {
                    $0.value == participantResponses.count
                }
                .map {
                    $0.key
                }
    ...
    static func solvePart1() {
        ...
        let sum = groupedAnswers.map({ $0.positiveResponses.count }).reduce(0, +)
        print("Sum: \(sum)")
    }
    ...
    static func solvePart2() {
        ...
        let sum = groupedAnswers.map({ $0.unanimousResponses.count }).reduce(0, +)
        print("Sum: \(sum)")
    }
}
