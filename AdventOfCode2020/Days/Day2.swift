...
struct Day2 {
    struct Policy {
        ...
        init(rawLine: Substring) {
            let sections = rawLine.split(separator: " ")
            let rangeValues = sections[0].split(separator: "-")
            range = Int(rangeValues[0])! ... Int(rangeValues[1])!
            character = Character(extendedGraphemeClusterLiteral: sections[1].first!)
            password = String(sections[2])
        }
        ...
        var isValidPart2: Bool {
            var found = false
            if password[password.index(password.startIndex, offsetBy: range.lowerBound-1)] == character {
                found = true
            }
            if password[password.index(password.startIndex, offsetBy: range.upperBound-1)] == character {
    ...
    static let policies: [Policy] = {
        ...
        for line in rawInput.split(whereSeparator: \.isNewline) {
            result.append(Policy(rawLine: line))
        }
        return result
    }()
    ...
    static func solvePart1() {
        ...
        print("Day 2 Part 1: \(policies.filter({ $0.isValidPart1 }).count)")
    }
    
    static func solvePart2() {
        print("Day 2 Part 2: \(policies.filter({ $0.isValidPart2 }).count)")
    }
}
