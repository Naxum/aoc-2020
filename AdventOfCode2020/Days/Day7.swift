...
struct Day7 {
    static let shinyGoldBagID = "shiny gold"
    
    static let bags: [String: Bag] = {
        var result = [String: Bag]()
        for line in rawInput.split(whereSeparator: \.isNewline) {
            let bag = Bag(rawLine: line)
            result[bag.name] = bag
            for contained in bag.contains.keys {
                bagContainers[contained, default: Set()].insert(bag.name)
            }
        }
        return result
    }()
    
    static var bagContainers = [String: Set<String>]()
    ...
    struct Bag {
        ...
        init(rawLine: Substring) {
            let primarySplit = rawLine.components(separatedBy: "contain")
            name = primarySplit[0].dropLast(5).trimmingCharacters(in: .whitespaces)
            for segment in primarySplit[1].split(whereSeparator: \.isPunctuation) {
                let trimmed = segment.trimmingCharacters(in: .whitespaces)
                if trimmed == "no other bags" { continue }
                let amount = Int(trimmed.prefix(1))!
                let name = trimmed.dropFirst().dropLast(4).trimmingCharacters(in: .whitespaces)
    ...
    static func solvePart1() {
        ...
        print("Bag count: \(bags.count)")
        // How many bag colors can eventually contain at least one shiny gold bag?
        var possibleContainers = Set<String>()
        var frontier = Set<String>()
        var checked = Set<String>()
        frontier.insert(shinyGoldBagID)
//        print("Containers of \(shinyGoldBagID): \(bagContainers[shinyGoldBagID])")
        while !frontier.isEmpty {
            let current = frontier.popFirst()!
            checked.insert(current)
            guard let containers = bagContainers[current] else { continue }
            for container in containers {
                possibleContainers.insert(container)
                if !checked.contains(container) {
                    frontier.insert(container)
                }
            }
        }
        print("Containers: \(possibleContainers.count)")
    }
    
    static func solvePart2() {
        print("Solving Day 7 Part 2")
        // How many individual bags are required inside your single shiny gold bag?
        var bagCount = 0
        func check(name: String, multiplier: Int) {
            for (bag, count) in bags[name]!.contains {
                bagCount += count * multiplier
                check(name: bag, multiplier: count * multiplier)
            }
        }
        check(name: shinyGoldBagID, multiplier: 1)
        print("Bags inside shiny gold bag: \(bagCount)")
    }
}
